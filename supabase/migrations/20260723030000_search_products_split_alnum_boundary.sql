-- Also split the search query at letter<->digit boundaries, not just at
-- punctuation. A query typed with zero separator (e.g. "k86" for the
-- "Axon Hearing Aid K 86" / tag "Axon K-86" product) previously became one
-- glued token "k86", and a prefix match on that single token can never hit
-- the product's actual indexed lexemes "k" and "86" (they're stored
-- separately because the name has a space and the tag has a hyphen) — a
-- prefix search only matches tokens that literally start with the query
-- string, and neither "k" nor "86" starts with "k86". Punctuated variants
-- ("k-86", "k 86") already worked before this migration since the existing
-- regexp_replace on line ~1 already turned punctuation into spaces; this
-- adds the same treatment for runs with no separator at all.
--
-- Checked every brand/tag/name in the catalog for letter-digit-adjacent
-- tokens before adding this (e.g. "GM700S", "B13", "2008b") — all are
-- model-code-style strings, not proper nouns a split would break.
