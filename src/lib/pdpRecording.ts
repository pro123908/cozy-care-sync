...
import { getActivePdpProduct } from "./pdpRecordingState";
...
//
// This file only exists as a dynamic-import target (see
// pdpRecordingState.ts's useSiteRecording()) — it is never statically
// imported from anywhere, on purpose. Its top-level import("@rrweb/record")
// pulls in a real dependency (~23KB gzipped); Vite/Rollup injects a
// <link rel="modulepreload"> for a dynamic import's target chunk based on
// static reachability from the entry graph, with no regard for the runtime
// condition wrapping the import() call — so as long as ANY eagerly-loaded
// file has a static import edge to this one, that preload hint (and the
// download) fires for every visitor on every page, sampled in or not. Moving
// the sampling check and the ProductDetail-facing setActivePdpProduct/
// getActivePdpProduct state into pdpRecordingState.ts (which has no such
// edge to this file except via a runtime-gated dynamic import) is what
// actually stops that — not the "check isSampledIn before doing anything"
// logic this file used to have, which looked like it should have worked and
// didn't.
//
...
// at that moment — see pdpRecordingState.ts's setActivePdpProduct — and is
// omitted (not overwritten to null) on chunks captured while off a product
// page, so the metadata row keeps the last-known product as its attribution.
// ---------------------------------------------------------------------------
...
// Called via a dynamic import from pdpRecordingState.ts's useSiteRecording(),
// only once that module has already confirmed this session sampled in — this
// file is never reached at all otherwise. Returns a cleanup function; the
// caller owns matching it to the component lifecycle (effect cleanup).
export function startSiteRecording(session: { id: string }): () => void {
  ...
  const flush = (isFinal = false) => {
    ...
    void uploadChunk(session.id, getActivePdpProduct(), thisChunk, toSend, totalSize, isFinal);
  };
